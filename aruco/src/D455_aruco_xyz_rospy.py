...
D455_arucos_pose = rospy.Publisher('D455_arucos_pose', PoseArray, queue_size=1)
D455_arucos_num = rospy.Publisher('D455_arucos_num', Int32, queue_size=1)
...
def calibrate_charuco():
    ...
    images = glob.glob(f'/home/scout/catkin_ws/src/2022TransportationRobot/aruco/src/D455_aruco_imgs/charuco*.png')
    # imsize = None
    for im in images:
...
def detect_marker(matrix, distortion):
    ...
    if cam.isOpened():
        while True:
            ...
            D455_arucos_pose.publish(arucos)
            D455_arucos_num.publish(len(arucos.poses))
...
def main():
    rospy.init_node("D455_aruco_xyz_rospy")
